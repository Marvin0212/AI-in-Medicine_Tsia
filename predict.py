...
Original file is located at
    https://colab.research.google.com/drive/1yjxuKqI-wJoZ28_sF7nFCgBIbH7l1jyM
"""
...
def data_preprocessing_short(ecg_lead):
        ...
        if remainder!=0:
          #take last bit to make a full data set 
          ecg_remainder = ecg_lead[len(ecg_lead)-desired_length:len(ecg_lead)]
          # Attach the remainder back to the ecg_lead
          ecg_array_processed = np.concatenate((ecg_lead[0:desired_length*amount_data], ecg_remainder))
          amount_data+=1
...
########make predictions
    ...
    #if one of the ecg lead subsets is positive classify the whole dataset positive
    offset=1
    for i in range(len(list_len)):
      afib=False
      for data_length in range(data_lengths[i]):
        if (stack_predictions[data_length+offset,0]<0.5):
          afib=True
